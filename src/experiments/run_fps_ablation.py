"""
FPS Ablation: Qwen2-VL-7B on Temporal Reasoning questions at 1 FPS and 2 FPS.
Baseline is 0.25 FPS (already in full_study results).
Also runs black-screen at each FPS level for VGG computation.
Conditions: original@1fps, black@1fps, original@2fps, black@2fps
"""
...
RESULTS_PATH = VDG_RESULTS_ROOT + "/experiments/fps_ablation_results.json"
VIDEO_DIR = VDG_DATA_ROOT + "/videomme_full/videos"
BLACK_DIR = VDG_DATA_ROOT + "/videomme_full/black_fps"
MODEL_ID = "Qwen/Qwen2-VL-7B-Instruct"
MAX_PIXELS = 128 * 128  # Reduced from 256*256 to fit more frames in VRAM
MIN_PIXELS = 28 * 28
FPS_LEVELS = [0.5, 1.0]  # 0.5 = 2x baseline, 1.0 = 4x baseline (vs 0.25 FPS)
...
def _print_summary(results):
    ...
    for fps in FPS_LEVELS:
        ...
        if orig_key in stats and black_key in stats:
            ...
            print(f"  {fps} FPS: orig={orig_acc:.3f} black={black_acc:.3f} VGG={vgg:.3f}")
