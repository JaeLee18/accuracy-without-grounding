"""
Shuffled-frames and single-frame ablation for InternVL2-8B on Video-MME.
...
RESULTS = {
    "shuffled":    VDG_RESULTS_ROOT + "/full_study/internvl2_shuffled_results.json",
    "singleframe": VDG_RESULTS_ROOT + "/full_study/internvl2_singleframe_results.json",
}
...
def load_video_frames(video_path, num_frames=NUM_FRAMES):
    """Load and transform frames, returning list of PIL images AND tensor."""
    vr = VideoReader(video_path, ctx=cpu(0), num_threads=1)
    total = len(vr)
    indices = np.linspace(0, total - 1, num_frames, dtype=int)
    pil_frames = [Image.fromarray(vr[i].asnumpy()).convert("RGB") for i in indices]
    return pil_frames
...
def run_inference(model, tokenizer, pil_frames, question, options):
    """Run InternVL2 inference with a list of PIL frames."""
    options_text = "\n".join(options)
    prompt_text = (f"{question}\n\n{options_text}\n\n"
                   "Answer with only the letter (A, B, C, or D) of the correct option.")
    tensor_frames = [_transform(f) for f in pil_frames]
    pixel_values = torch.stack(tensor_frames).to(torch.bfloat16).cuda()
    num_patches_list = [1] * len(pil_frames)
...
def run_condition(model, tokenizer, samples, condition):
    ...
    print(f"\n=== InternVL2 {condition.upper()} === Remaining: {len(work)} / {len(samples)}")
    ...
    for sample in pbar:
        ...
        try:
            pil_frames = load_video_frames(video_path, NUM_FRAMES)

            if condition == "shuffled":
                frame_seed = hash(sample["videoID"]) + SEED
                rng = random.Random(frame_seed)
                rng.shuffle(pil_frames)
            elif condition == "singleframe":
                mid = len(pil_frames) // 2
                pil_frames = [pil_frames[mid]] * len(pil_frames)

            pred = run_inference(model, tokenizer, pil_frames,
...
def main():
    ...
    for condition in ["shuffled", "singleframe"]:
        run_condition(model, tokenizer, samples, condition)
